...
// forward declaration
STATUS jitter_buffer_internalParse(PJitterBuffer pJitterBuffer, BOOL bufferClosed);
...
STATUS jitter_buffer_free(PJitterBuffer* ppJitterBuffer)
{
    ...
    jitter_buffer_internalParse(pJitterBuffer, TRUE);
...
STATUS jitter_buffer_push(PJitterBuffer pJitterBuffer, PRtpPacket pRtpPacket, PBOOL pPacketDiscarded)
{
    ...
    CHK_STATUS(jitter_buffer_internalParse(pJitterBuffer, FALSE));
...
STATUS jitter_buffer_internalParse(PJitterBuffer pJitterBuffer, BOOL bufferClosed)
{
    ...
    if (bufferClosed && curFrameSize > 0) {
        ...
        if (UINT16_DEC(index) == lastNonNullIndex) {
            ...
            CHK_STATUS(jitter_buffer_dropBufferData(pJitterBuffer, startDropIndex, lastNonNullIndex, pJitterBuffer->headTimestamp));
        } else {
...
STATUS jitter_buffer_fillFrameData(PJitterBuffer pJitterBuffer, PBYTE pFrame, UINT32 frameSize, PUINT32 pFilledSize, UINT16 startIndex, UINT16 endIndex)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    UINT16 index = startIndex;
